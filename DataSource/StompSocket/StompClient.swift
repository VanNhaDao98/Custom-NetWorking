...
open class StompClient {
    ...
    private func processString(string: String) {
        ...
        if let commandValue = contents.first, let command = ResponseCommand(rawValue: commandValue) {
    ...
    private func sendFrame(command: Command, header: [CommandHeader: String]?, body: String?) {
        ...
        print("[StompClient] Websocket sendFrame: \(frameString)")
    ...
    private func reconnectLogic(request: URLRequest,
        ...
        if !isConnected || force {
            openSocketWithUrlRequest(urlRequest: request, connectionHeaders: connectionHeaders)
        }
    }
